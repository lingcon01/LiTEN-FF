...
class NabEnergyTable:
    ...
    def get_energy(self, Z):
        """
        输入: 原子序数（int 或 Tensor/List of int）
        输出: 对应的能量值
        """
        try:
...
class LiTENCalculator(Calculator):
    ...
    def __init__(self, model_name, is_pbc, is_batch, atoms=None, data=None, device=None, dtype=None, unit_energy=None,
        ...
        if self.model_name == 'nabladft' and self.is_pbc is not True:
            model_path = model_checkpoints['LiTEN_Nab']
        elif self.model_name == 'spice' and self.is_pbc is not True:
            model_path = model_checkpoints['LiTEN_SPICE']
        elif self.model_name == 'spice' and self.is_pbc:
            model_path = model_checkpoints['LiTEN_PBC']
        else:
            raise ValueError("nablaDFT is not supportted pbc")

        self.model = torch.load(model_path, weights_only=False, map_location=self.device).eval()
